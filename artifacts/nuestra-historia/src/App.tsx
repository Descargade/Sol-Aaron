...
function PublicStoryPage() {
  ...
  return <Shell>
    ...
    <main>
      ...
      <footer className="public-footer"><div className="footer-heart"><Heart size={18} fill="currentColor" /></div><p>Hecho de recuerdos por Sol y Aaron</p><span>Mendoza · {new Date().getFullYear()}</span></footer>
    </main>
...
function AlbumForm({ item, onClose }: { item: Album | null; onClose: () => void }) { const create = useCreateAlbum(); const update = useUpdateAlbum(); const qc = useQueryClient(); return <Modal title={item ? 'Editar álbum' : 'Nuevo álbum'} onClose={onClose}><form onSubmit={(e) => { e.preventDefault(); const f = new FormData(e.currentTarget); const payload = { name: String(f.get('name')), description: String(f.get('description')), coverUrl: String(f.get('coverUrl') || '') || null, sortOrder: Number(f.get('sortOrder') || 0) }; const done = () => { qc.invalidateQueries({ queryKey: getListAlbumsQueryKey() }); qc.invalidateQueries({ queryKey: getGetAdminStoryQueryKey() }); onClose(); }; item ? update.mutate({ id: item.id, data: payload }, { onSuccess: done }) : create.mutate({ data: payload }, { onSuccess: done }); }}><Field name="name" label="nombre" defaultValue={item?.name} required /><Field name="description" label="descripción" defaultValue={item?.description} textarea /><Field name="coverUrl" label="url de portada" defaultValue={item?.coverUrl || ''} /><button className="button dark full" data-testid="button-save-album"><Check size={16} /> guardar álbum</button></form></Modal>; }
