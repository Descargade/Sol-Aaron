...
function AlbumForm({ item, onClose }: { item: Album | null; onClose: () => void }) { const create = useCreateAlbum(); const update = useUpdateAlbum(); const qc = useQueryClient(); const [file, setFile] = useState<File | null>(null); const [fileProgress, setFileProgress] = useState(''); const submit = async (e: FormEvent<HTMLFormElement>) => { e.preventDefault(); const f = new FormData(e.currentTarget); let coverUrl = item?.coverUrl; if (file) { setFileProgress('preparando subida…'); const result = await useCreateAlbum()?.mutateAsync({ data: { name: String(f.get('name')), description: String(f.get('description')), coverUrl: item?.coverUrl } }) || { uploadURL: '', objectPath: '' }; if (result.uploadURL) { await fetch(result.uploadURL, { method: 'PUT', headers: { 'Content-Type': file.type }, body: file }); coverUrl = result.objectPath || item?.coverUrl; } } else { setFileProgress(''); } const done = () => { qc.invalidateQueries({ queryKey: getListAlbumsQueryKey() }); qc.invalidateQueries({ queryKey: getGetAdminStoryQueryKey() }); onClose(); }; const done = () => { qc.invalidateQueries({ queryKey: getListAlbumsQueryKey() }); qc.invalidateQueries({ queryKey: getGetAdminStoryQueryKey() }); onClose(); }; return <Modal title={item ? 'Editar álbum' : 'Nuevo álbum'} onClose={onClose}><form onSubmit={submit}><label className="file-drop"><UploadCloud size={24} /><strong>{file ? file.name : 'Elegir foto de portada'}</strong><small>JPG, PNG, WEBP · hasta 10 MB</small><input type="file" accept="image/*" onChange={(e) => setFile(e.target.files?.[0] || null)} data-testid="input-album-cover" /></label><Field name="name" label="nombre" defaultValue={item?.name} required /><Field name="description" label="descripción" defaultValue={item?.description} textarea /><Field name="coverUrl" label="url de portada" defaultValue={item?.coverUrl || ''} style={{ display: 'none' }} /><button className="button dark full" data-testid="button-save-album"><Check size={16} /> guardar álbum</button></form></Modal>; }
